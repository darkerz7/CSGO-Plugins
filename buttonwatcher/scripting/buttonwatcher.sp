...
Handle g_hCookie_BBans = null, g_hCookie_BBansLength = null, g_hCookie_BBansIssued = null, g_hCookie_BBansBy = null, g_hCookie_Buttons = null;
...
ConVar g_hCvar_ButtonsEnabled, g_hCvar_ButtonsTimer;

bool g_bButtonsEnabled = true;

bool isMapRunning;

int g_aButtons[MAX_EDICTS], g_iMaxButtons = 0;
...
public Plugin myinfo =
{
    ...
    description = "Track button presses and with ban functionalities",
    version = "2.1.0", // New syntax
    url = ""
};
...
public void AdminMenu_Commands_Handler(Handle hMenu, TopMenuAction hAction, TopMenuObject hObjID, int iParam1, char[] sBuffer, int iMaxlen)
{
    switch (hAction)
    {
        ...
        {
            FormatEx(sBuffer, iMaxlen, "%s", "Buttons Commands", iParam1);
        }
...
public void Handler_BBanList(Handle hMenu, TopMenuAction hAction, TopMenuObject hObjID, int iParam1, char[] sBuffer, int iMaxlen)
{
    switch (hAction)
    {
        case TopMenuAction_DisplayOption:
        {
            FormatEx(sBuffer, iMaxlen, "%s", "List Buttons Banned Clients", iParam1);
        }
...
public void Handler_BBan(Handle hMenu, TopMenuAction hAction, TopMenuObject hObjID, int iParam1, char[] sBuffer, int iMaxlen)
{
    switch (hAction)
    {
        case TopMenuAction_DisplayOption:
        {
            FormatEx(sBuffer, iMaxlen, "%s", "Buttons Ban a Client", iParam1);
        }
        case TopMenuAction_SelectOption:
        {
            Menu_BBan(iParam1);
...
public void Handler_UnBBan(Handle hMenu, TopMenuAction hAction, TopMenuObject hObjID, int iParam1, char[] sBuffer, int iMaxlen)
{
    switch (hAction)
    {
        case TopMenuAction_DisplayOption:
        {
            FormatEx(sBuffer, iMaxlen, "%s", "Buttons Unban a Client", iParam1);
        }
        case TopMenuAction_SelectOption:
        {
            Menu_UnBBan(iParam1);
...
public int MenuHandler_Menu_BBan_List(Menu hMenu, MenuAction hAction, int iParam1, int iParam2)
{
    switch (hAction)
    {
        ...
        case MenuAction_Select:
        {
            ...
            if (iTarget != 0)
            {
                Menu_BBan_ListTarget(iParam1, iTarget);
            }
            else
            {
...
void Menu_BBan_ListTarget(int iClient, int iTarget)
{
    ...
    if (!(iBanIssuedDate == 0))
    {
        FormatTime(sBufTimeIss, sizeof(sBufTimeIss), "%m/%d/%Y | %H:%M",iBanIssuedDate);
    }
    else
    {
        FormatEx(sBufTimeIss, sizeof(sBufTimeIss), "Unknown");
    }
    ...
    switch (hAction)
    {
        ...
        case MenuAction_Select:
        {
            ...
            if (iTarget != 0)
            {
                ...
                {
                    BBanClient(iTarget, "-1", iParam1);
                }
                else if (strcmp(sOption, "0") == 0)
                {
                    BBanClient(iTarget, "0", iParam1);
                }
                else
                {
...
void Menu_UnBBan(int iClient)
{
    ...
    if (!iBannedClients)
    {
        hUnBBanMenu.AddItem("", "No Banned Clients.", ITEMDRAW_DISABLED);
    }
...
public Action OnButtonUse(int iButton, int iActivator)
{
    ...
    {	
        if (EntWatch_IsSpecialItem(iButton))
        {
            return Plugin_Continue;
        }
    
        if (g_bBBans[iActivator])
        {
...
public Action Command_BBan(int iClient, int iArgs)
{
    ...
    if (GetCmdArgs() > 1)
    {
        ...
        if (StringToInt(sLen) <= -1)
        {
            BBanClient(iTarget, "-1", iClient);
        }
        ...
        {
            BBanClient(iTarget, sLen, iClient);
        }
        return Plugin_Handled;
    }
...
public Action Command_BBanlist(int iClient, int iArgs)
{
    ...
    if (bFirst)
    {
        CReplyToCommand(iClient, "[EYE] Currently BBanned: No players found.");
    }
...
{
    if (g_bButtons[iClient] == false)
    {
        g_bButtons[iClient] = true;
    }
    else
    {
        g_bButtons[iClient] = false;
...
public Action PressBTN(const char[] output, int caller, int activator, float delay)
{
    ...
    if (EntWatch_IsSpecialItem(caller))
    {
        return Plugin_Continue;
    }
